...
class Appointment:
    ...
    def __str__(self):
        ...
        return f"{self.client_name}{self.client_phone:>23}{self.day_of_week:>10}{start_hour:>4}:00  -  {end_hour:>2}:00{self.get_appt_type_desc():>20}" 
...
def print_menu():
    ...
    return int(choice) if choice.isdigit() and choice in '12345679' else None  # Return the selected option if valid.
...
def show_appointments_by_name(appointments, name):
    ...
    for appt in appointments:
        if name.capitalize() in appt.get_client_name().capitalize():  # Case-insensitive search by name.
...
def save_appointments(appointments):
    ...
    if os.path.exists(filename):  # Check if file exists.
        if input(f"{filename} exists. Overwrite? (Y/N): ").upper() != 'Y':  # Confirm overwrite.
            print("Save canceled.")
...
def main():
    ...
    elif load.upper() != 'Y' and load.upper() != 'N':  # Validate input.
    ...
    while True:
        ...
        if choice == 1:  # Option to schedule an appointment.
            ...
            appt_type = int(input("Enter appointment type (1: Mens Cut $40, 2: Ladies Cut $60, 3: Mens Colouring $40, 4: Ladies Colouring $80): "))
            ...
            print(f"{"Client Name":>10}{"Phone":>10}{"Day":>12}{"Start":>12}{"End":>8}{"Type":>12}")
            print("-"*80)
            ...
            if appt and appt.get_client_phone() != "":
            ...
            if input("Save appointments before exiting? (Y/N): ").upper() == 'Y':
